/* global React */
const { useRef: useRefSec, useState: useStateSec } = React;
const TabsSec = window.RadixTabs;
const TooltipSec = window.RadixTooltip;
const HoverCardSec = window.RadixHoverCard;
...
function ProjectTechIcon({ tech }) {
  return (
    <HoverCardSec.Root openDelay={120} closeDelay={80}>
      <HoverCardSec.Trigger asChild>
      ...
      </HoverCardSec.Trigger>
      <HoverCardSec.Portal>
        <HoverCardSec.Content
          sideOffset={10}
        ...
        >
          ...
          {tech.name}
          <HoverCardSec.Arrow className="fill-white/95 dark:fill-neutral-900/95" />
        </HoverCardSec.Content>
      </HoverCardSec.Portal>
    </HoverCardSec.Root>
  );
...
function GalleryCarousel({ images }) {
  const [index, setIndex] = useStateSec(0);
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.14), inset 0 -32px 48px -36px rgba(255,255,255,0.22)",
        }}
      />
      ...
      {images.map((image, imageIndex) => (
        <TabsSec.Content key={image.src} value={String(imageIndex)} className="absolute inset-0 m-0 outline-none">
          <img
            src={image.src}
            alt={image.alt}
      ...
      <TooltipSec.Provider delayDuration={80}>
        <TooltipSec.Root>
          <TooltipSec.Trigger asChild>
            <button
              type="button"
              onClick={() => move(-1)}
          ...
          <TooltipSec.Portal>
            <TooltipSec.Content
              side="top"
              sideOffset={6}
              className="px-2 py-1 rounded-md bg-white/95 dark:bg-neutral-900/95 border border-neutral-200 dark:border-neutral-800 text-neutral-800 dark:text-neutral-100 text-[11px] shadow-sm"
            >
              Previous
              <TooltipSec.Arrow className="fill-white/95 dark:fill-neutral-900/95" />
            </TooltipSec.Content>
          </TooltipSec.Portal>
        </TooltipSec.Root>
...
function ProjectMedia({ media, side, title }) {
  ...
  const frameClass =
    "relative w-full max-w-md rounded-lg overflow-hidden border border-neutral-200 dark:border-neutral-800/80 shadow-lg shadow-black/20 " +
    align;
    ...
    return (
      <div className={mediaWrapClass}>
        <div className={frameClass}>
          <GalleryCarousel images={media.images} />
        </div>
        {technologies.length > 0 && (
          <div className={"mt-4 flex flex-wrap items-center gap-4 " + iconAlign}>
            {technologies.map((tech) => (
              <ProjectTechIcon key={tech.name} tech={tech} />
            ))}
          </div>
        )}
      </div>
...
Object.assign(window, { AboutSection, ProjectsSection, ContactSection });
